...
/*//////////////////////////////////////////////////////////////////////////
add by ybt at 2012_10_24
功能：根据文件名得到该文件MD5值， 
参数：
    md5      : [OUT]文件MD5，长度须大于或等于MD5的长度32 + 1(for '\0')
    fileName : [IN]文件名
注：本函数延续其原来的代码风格。同时，原来的代码应该同时适用于windows、linux系统，故文件操作类采用CRT函数，而不采用Win API
//////////////////////////////////////////////////////////////////////////*/
static TCHAR* hexstr = _T("0123456789ABCDEF");
bool MD5File(TCHAR *md5, const TCHAR *fileName)
{
	MD5_CTX			md5_ctx;
	FILE			*f;
	...
	{
		//2012_10_29
		//1、fread(buf, 1, 2048, f); 每次读取一个字节，读取2048次
		//2、fread(buf, 2048, 1, f); 每次读取2048个字节，读取1次
		//看到这，是不是觉得两种方式效果应该一样，而且觉得第一种方法简直是弱爆了，效率节操什么的都没有了，什么水平？ 
		//然后我也是这么想的，用了第二种方法，最后发现计算的MD5完全不对，最后发现是这里的问题。
		//其实是不一样的，根本在于它们的返回值不同（之前以为fread返回的是字节数）。
		//第一种，返回值在0~2048之间；第二种，返回值0~1。也就是，fread返回的并不是字节数。
		//附：测试结果(测试10次以上，环境Win7 CPU : 2.93GHZ, MEM : 4GB)，读取一个1.04G的文件(只执行fread)，第一种方式约花费1950ms，第二种方式2950ms。第一种方式却是优于第二种。
	...
	{
		*md5++ = hexstr[md5_ctx.digest[cnt] >> 4];		//取高4位
		*md5++ = hexstr[md5_ctx.digest[cnt] & 0x0F];	//取低4位
	}
	*md5 = 0;
