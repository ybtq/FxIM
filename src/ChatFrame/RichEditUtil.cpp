...
int RichEdit_GetWindowText(ITextServices * pTextServices, wstring& strText)
{
	...
	strText.resize(nLength+1, 0);
	nLength = RichEdit_GetWindowText(pTextServices, &strText[0], nLength+1);
...
int RichEdit_GetTextRange(ITextServices * pTextServices, CHARRANGE * lpchrg, wstring& strText)
{
	...
	LONG nLen = lpchrg->cpMax - lpchrg->cpMin;
	strText.resize(nLen + 1, 0);

	TEXTRANGEW tr = { 0 };
	tr.chrg = *lpchrg;
	tr.lpstrText = &strText[0];
...
// 设置默认字体
void RichEdit_SetDefFont(ITextServices * pTextServices, LPCWSTR lpFontName,	
						 ...
						 BOOL bItalic, BOOL bUnderLine, BOOL bIsLink)
{
	...
	{
		cf.dwMask = cf.dwMask | CFM_FACE;
		wcscpy(cf.szFaceName, lpFontName);
	}
						 ...
						 COLORREF clrBg, BOOL bAutoScale, int nReservedWidth, IImageOle** ppImageOle/* = NULL*/)
{
	...
	WCHAR cProtocol[16] = {0};
	wcsncpy(cProtocol, lpszFileName, 7);
	if ((wcsicmp(cProtocol, L"http://") != 0) && ::GetFileAttributesW(lpszFileName) == 0xFFFFFFFF)
		return FALSE;
	...
	if (ppImageOle) *ppImageOle = pImageOle;
Ret0:
	if (pOleObject != NULL)
		pOleObject->Release();
...
// 获取文本
void RichEdit_GetText(ITextServices * pTextServices, wstring& strText, BOOL bIncludeOleInfo)
{
	REOBJECT reobject;
	LONG nFaceId, nPos = 0;
...
// 替换选中文本
void RichEdit_ReplaceSel(ITextServices * pTextServices, LPCWSTR lpszNewText,	
						 ...
						 int nStartIndent, BOOL bCanUndo/* = FALSE*/)
{
	...
	lEndChar = lStartChar + wcslen(lpszNewText);
